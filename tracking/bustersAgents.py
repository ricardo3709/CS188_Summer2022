...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Find the closest postion of max belief
        min_distance = float("inf")
        for belief in livingGhostPositionDistributions:
            distance = self.distancer.getDistance(
                pacmanPosition, belief.argMax())
            if distance < min_distance:
                min_distance = distance
                min_position = belief.argMax()

        # Find action which lead closest successor state
        target_min_distance = float("inf")
        for action in legal:
            new_position = Actions.getSuccessor(pacmanPosition, action)
            new_distance = self.distancer.getDistance(
                new_position, min_position)
            if new_distance < target_min_distance:
                target_min_distance = new_distance
